...
# Fungsi utama per tahun (file JSON)
def process_year(path_str):
    import matplotlib
    matplotlib.use('Agg')  # agar matplotlib aman untuk multiprocess
...
if __name__ == "__main__":
    json_files = sorted(INPUT_DIR.glob("preprocessed_abstracts_lemmatization_*.json"))
    paths = [str(p) for p in json_files]  # convert ke string agar aman di Windows

    with ProcessPoolExecutor(max_workers=16) as executor:  # contoh pakai 8 core
